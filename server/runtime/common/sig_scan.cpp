...
namespace DemoTracerRuntime::Sig
{
    ...
    {
        const auto address = reinterpret_cast<uintptr_t>(interfacePtr);
        if (address < 0x10000u || address + sizeof(void *) < address)
            return {};
        void *vtable = nullptr;
#if defined(_WIN32)
        SIZE_T read = 0;
        if (!ReadProcessMemory(GetCurrentProcess(), interfacePtr, &vtable, sizeof(vtable), &read) ||
            read != sizeof(vtable))
            return {};
#else
        std::memcpy(&vtable, interfacePtr, sizeof(vtable));
#endif
        if (!vtable)
            return {};

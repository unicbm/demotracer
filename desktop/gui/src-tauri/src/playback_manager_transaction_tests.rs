...
const CONTROLLER: &str = "addons/counterstrikesharp/plugins/BotControllerImpl/BotControllerImpl.dll";
const OLD_HIDER: &str = "addons/counterstrikesharp/plugins/DemoTracerBotHider/DemoTracerBotHider.dll";
...
impl InstallFixture {
    fn new() -> Self {
        let fixture = Self(std::env::temp_dir().join(format!("dtr-install-{}", uuid::Uuid::new_v4())));
        fixture.write("gameinfo.gi", b"GameInfo");
        fixture
    }

    fn game(&self) -> PathBuf { self.0.join("cs2/game/csgo") }
    fn local(&self) -> PathBuf { self.0.join("local") }
    ...
    fn install(&self, version: &str) -> CommandResult<PlaybackInstallResultDto> {
        install_package_bytes(&self.local(), self.game().to_str().unwrap(),
            &package_bytes(version), version, "test")
    }
    fn rollback(&self) -> CommandResult<PlaybackInstallResultDto> {
        rollback_latest(&self.local(), self.game().to_str().unwrap())
    }
    fn state_path(&self) -> PathBuf { install_state_path(&self.local(), &self.game()) }
    fn snapshot(&self) -> BTreeMap<String, String> {
        collect_normal_files(&self.game(), MAX_ZIP_ENTRIES).unwrap().into_iter()
            .map(|path| (normalized_receipt_path(&path.strip_prefix(self.game()).unwrap().to_string_lossy()),
...
fn package_bytes(version: &str) -> Vec<u8> {
    ...
    let files = REQUIRED_RECEIPT_PATHS.iter().map(|path| {
        let content = format!("{version}:{path}").into_bytes();
        zip.start_file(*path, options).unwrap();
        zip.write_all(&content).unwrap();
        ReceiptFileWire {
            path: path.to_string(), component: receipt_component(path).unwrap().to_string(),
            size: content.len() as u64, sha256: sha256_hex(&content),
        }
    }).collect();
    let receipt = InstallReceiptWire {
        schema_version: 1, product: "CS2 DemoTracer Playback Bundle".to_string(),
        bundle_version: version.to_string(), git_commit: None, platform: "windows-x64".to_string(),
        compatibility: embedded_playback_contract().unwrap(), files,
    };
    zip.start_file(INSTALL_RECEIPT_RELATIVE_PATH, options).unwrap();
    zip.write_all(&serde_json::to_vec(&receipt).unwrap()).unwrap();
...
fn provider_migration_and_rollback_preserve_user_data_and_original_code() {
    ...
    fixture.write("addons/counterstrikesharp/plugins/Unrelated/plugin.dll", b"unrelated");
    let original = fixture.snapshot();
    fixture.install("1.2.2").unwrap();
    assert!(!fixture.game().join(OLD_HIDER).exists());
    assert_ne!(fs::read(fixture.game().join(CONTROLLER)).unwrap(), b"upstream controller");
    assert_eq!(fs::read(fixture.game().join(USER_CONFIG)).unwrap(), b"user settings");
    assert_eq!(fs::read(fixture.game().join(RECORDING)).unwrap(), b"user recording");
...
fn later_external_overwrite_blocks_rollback_and_repair_preserves_that_baseline() {
    ...
    assert_eq!(fixture.rollback().unwrap_err().code, "playback_modified_since_install");
...
fn missing_backup_is_detected_before_rollback_changes_any_installed_file() {
    ...
    let original = fixture.snapshot();
    fs::remove_file(Path::new(&installed.backup_path).join("files").join(CONTROLLER.to_ascii_lowercase())).unwrap();
...
fn install_rejects_changed_event_semantics_even_when_tick_size_and_abi_match() {
    ...
    let package = extract_and_validate_package(&bytes, &fixture.0.join("payload"), "1.2.2").unwrap();
    let mut receipt = package.receipt;
    receipt.compatibility.bot_controller.replay_tick_event_tail = "drop_events".to_string();
    assert_eq!(validate_receipt_contract(&receipt, "1.2.2").unwrap_err().code,
    ...
    let controller = old["compatibility"]["bot_controller"].as_object_mut().unwrap();
    for key in ["public_control_api", "replay_tick_bytes", "replay_tick_event_tail", "managed_provider_version"] {
